...
router.post('/', async (req, res) => {
  try {
    ...
    return res.status(201).json({ status: 'created', message: 'New Cart Created', cart: newCart });
  } catch (error) {
...
router.get('/:cid', async (req, res) => {
  try {
    ...
    return res.status(500).json({ status: 'error', error: 'Error getting cart products' });
  }
});
...
router.post('/:cid/product/:pid', async (req, res) => {
  try {
    ...
    if (!product) {
      return res.status(404).json({ status: 'error', error: 'Product not found. You must enter the ID of an existing product in the products.json file' });
    }
...
router.delete('/:cid', async (req, res) => {
  try {
    ...
    return res.status(200).json({ status: 'success', message: 'Cart successfully deleted' });
  } catch (error) {
    return res.status(500).json({ status: 'error', error: 'Error deleting cart' });
  }
});
